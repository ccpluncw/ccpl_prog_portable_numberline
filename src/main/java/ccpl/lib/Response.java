...
public class Response implements KeyListener, ActionListener, ChangeListener {
    ...
    public synchronized long getTimedKeyResponse(PrintWriter log) { //remove
        log.append("Response Method Started\n"); //remove
         ...
         **/
        log.append("Response Loop Starting\n"); //remove
        while (getUserChoice() == '~') {
            timeB = new Date().getTime();
            if (print) {
        ...
        log.append("Response Loop Finished\n"); //remove
    ...
    public synchronized long getTimedTwoButtonResponse(String labelA, String labelB) {
        ...
        choice = JOptionPane.showOptionDialog(null, "Please Choose an Option",
                "Response", JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null, buttonLabels, buttonLabels[0]);
    ...
    public synchronized void getTimedSliderNoThumbResponse(JFrame parent, int min, int max, int initial,
        ...
        j1.setFont(f2);
        j2.setFont(f2);
        labelTable.put(new Integer(min), j1);
        labelTable.put(new Integer(max), j2);
    ...
    */
    ...
    public synchronized void getTimedSliderResponse(JFrame parent, int min, int max, int initial,
        ...
        Hashtable labelTable = new Hashtable();
        labelTable.put(new Integer(min), new JLabel(lowLabel));
        labelTable.put(new Integer(max), new JLabel(highLabel));
        slider.setLabelTable(labelTable);
    ...
    public void getFixedTimeNumPadResponse(int responseTimeLimit){
        ...
        startTime = new Date().getTime();
            long timeSpent = 0;
            while(getInputDone() == false && timeSpent < responseTimeLimit){
...
public synchronized void getTimedTextResponseJustified(JFrame parent, String info, int columns, String just) {
    ...
    public synchronized void getTimedRadioButtonResponse(JFrame parent, String info, 
            String[] options) {
    ...
    public long getTimedNumberLineResponse(NumberLine lPanel, boolean useMouse) {
        ...
        while (lPanel.isHandleDragged() != true) {
          try {
            Thread.sleep(20);
...
//    g.fillRect(0,0,d.width,d.height);
    ...
    public boolean checkResponseAccuracy(int probeType, char response){
        if (probeType == 0) {
            if (response == sameChoice) {
                correct = true;
            } else {
                correct = false;
            }
        } else {
            if (response == diffChoice) {
                correct = true;
            } else {
                correct = false;
            }
    ...
    public boolean checkResponseAccuracy(String resp, int[] correctResp){
        if(correctResp[0] == 0){
            if(resp.charAt(0) == sameChoice && resp.length() == correctResp[1]){
                correct = true;
            }
            else{
                correct = false;
            ...
            if(resp.charAt(0) == diffChoice && resp.length() == correctResp[1]){
                correct = true;
            }
            else{
                correct = false;
...
//        }
    ...
    public synchronized void giveFeedbackSwitch(JFrame frame,
        ...
        if (correct && feedback.equalsIgnoreCase("ALL")) {
            ...
            wOffset = fm.stringWidth(feedbackLabel.getText()) / 2;
            hOffset = fm.getHeight() / 2;
            g.drawString("Correct", d.width / 2 - wOffset,
                    d.height / 2 + hOffset);
        }
        else if(userChoice == '*'){
        ...
        // and the feedback is either all or incorrect, display message
            ...
            wOffset = fm.stringWidth(feedbackLabel.getText()) / 2;
            hOffset = fm.getHeight() / 2;
            g.drawString("Incorrect", d.width / 2 - wOffset,
                    d.height / 2 + hOffset);
        }
