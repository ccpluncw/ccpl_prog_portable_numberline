...
public class Unit {
    ...
    /*public static void main(String[] args){
        Unit fract = new Unit("2/3");
        Unit odd = new Unit("2 in 4");
        Unit deci = new Unit("6.25");
        Unit intUnit = new Unit("7");
        System.out.print(fract + " " + odd +" " + deci +  " " +intUnit);
        System.out.println(fract.getType() + " " + odd.getType() + " " + deci.getType() + " " + intUnit.getType());
    }*/

    public static enum UNITTYPE {INT, DECI, FRACT, ODDS}
    private static String intRegex = "^(\\d)+$";
    private static String deciRegex = "^(\\d)*(.(\\d)+)?$";
    private static String fractRegex = "^(\\d)+\\s*/\\s*(\\d)+$";
    private static String oddRegex = "^(\\d)+\\s*in\\s*(\\d)+$";
    ...
    private static JLabel getLabel(String val, Font labelFont, Color foreground){
        JLabel unitLabel = new JLabel();
        unitLabel.setForeground(foreground);
